...
def run_priceseqfilter(input_fqs):
    ...
    async_handler.launch_aws_upload(RESULT_DIR + "/" + PRICESEQFILTER_OUT1, SAMPLE_S3_OUTPUT_PATH + "/")
    if len(input_fqs) == 2:
        execute_command("mv %s %s" % (output_files[1], os.path.join(RESULT_DIR, PRICESEQFILTER_OUT2)))
        async_handler.launch_aws_upload(RESULT_DIR + "/" + PRICESEQFILTER_OUT2, SAMPLE_S3_OUTPUT_PATH + "/")
...
def run_fq2fa(input_fqs):
    ...
    write_to_log("finished job")
    async_handler.launch_aws_upload(RESULT_DIR + "/" + FQ2FA_OUT1, SAMPLE_S3_OUTPUT_PATH + "/")
    if len(input_fqs) == 2:
        async_handler.launch_aws_upload(RESULT_DIR + "/" + FQ2FA_OUT2, SAMPLE_S3_OUTPUT_PATH + "/")
