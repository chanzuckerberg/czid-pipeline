    ...
    for mapping_file in mapping_files:
        with gzip.open(mapping_file, 'rb') as mapf:
            for line in mapf:
                ...
                lines += 1
                if lines % 100000 == 0:
                    print("%3.1f M lines. %s, %s" % (lines / 1000000.0,
                                                     fields[0], fields[2]))
...
class Curate_accession2taxid(Base):
    def run(self):
        ...
        output_mapping_file = os.path.join(DEST_DIR,
                                           'curated_accession2taxid.txt')
        curate_taxon_dict(nt_file_local, nr_file_local, mapping_files_local,
                          output_mapping_file)
        ...
        output_s3_file = os.path.join(output_s3_path,
                                      os.path.basename(output_db_file))

        # Record difference with old accession2taxid
        previous_mapping_s3 = arguments.get('--previous_mapping')
        if previous_mapping_s3:
            print("Computing diff")
            previous_mapping_local = os.path.join(dest_dir,
                                                  "previous_mapping.db.gz")
            execute_command("aws s3 cp --quiet %s %s" %
                            (previous_mapping_s3, previous_mapping_local))
            print("Starting gunzip")
            execute_command("gunzip -fk %s" % previous_mapping_local)
            print("Starting shelve open")
            previous_mapping = shelve.open(
                os.path.splitext(previous_mapping_local)[0])
            ...
            diff_accessionids = {
                'old_file': previous_mapping_s3,
                'new_file': output_s3_file,
                'added': added_accessionids,
                'removed': removed_accessionids
            }
            diff_accessionids_file = os.path.join(dest_dir,
                                                  "accession_diff.txt")
            print("Starting json dump")
            with open(diff_accessionids_file, 'wb') as f:
                json.dump(diff_accessionids, f)
            execute_command("aws s3 cp --quiet %s %s/" %
                            (diff_accessionids_file, output_s3_path))

        # Upload result and record versions
        print("Uploading result to S3")
        execute_command(
            "aws s3 cp --quiet {output_db_file} {output_s3_file}".format(
                output_db_file=output_db_file, output_s3_file=output_s3_file))
        upload_version_tracker(
            mapping_files_sources +
            [arguments['--nt_file'], arguments['--nr_file']],
            'accession2taxid',
