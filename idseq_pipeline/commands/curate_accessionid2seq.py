...
class Curate_accessionid2seq(Base):
    def run(self):
        ...
        local_input_file = os.path.join(dest_dir,
                                        os.path.basename(s3_input_file))
        local_output_file = os.path.join(dest_dir,
                                         os.path.basename(s3_output_file))
        ...
        # copy the data back
        execute_command("aws s3 cp %s %s" % (local_output_file,
                                             s3_output_file))
